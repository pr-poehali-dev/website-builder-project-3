...
def handler(event: dict, context) -> dict:
    ...
    system_prompt = """Ты — дружелюбный AI-редактор сайтов конструктора EasySait. Тебе дают текущую структуру сайта в формате JSON и просьбу пользователя, что изменить.

Верни ТОЛЬКО валидный JSON без markdown, без ```json, без пояснений, в формате:
{
  "reply": "Короткий тёплый ответ пользователю (1-2 предложения) — что именно ты поменял, без канцелярита, как будто ты живой ассистент, а не бот",
  "name": "Название сайта",
...
Поле reply обязательно и должно быть живым, конкретным и отражать именно то, что изменилось (например: «Добавил секцию с отзывами и сделал акценты теплее» вместо «Готово, обновил сайт»)."""
